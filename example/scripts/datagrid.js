...
function getTaxoniumLink(taxoniumURL, cluster) {
  let link = '<a href="https://taxonium.org/?' + taxoniumURL;
...
function clusterObjs(items, taxoniumURL) {
  el = {
    'cid': items[0],
    ...
    'taxlink': getTaxoniumLink(taxoniumURL, items[0]),
  };
  return el;
}
...
function setCols() {
  const cols = [
    ...
    {id: 'samplecol', name: `<span data-toggle='tooltip' title='${tooltipText[11]}'>Samples</span>`, field: 'samplecol', minWidth: 100, sortable: true, sorter: sorterStringCompare, customTooltip: {useRegularTooltip: true}, formatter: tooltipFormatter},
  ];
  return cols;
}
...
function setGridView() {
  ...
  grid.onDblClick.subscribe((e, p) => {
    // p.row, p.cell. column 7 & 8 = Potential Origin & Index. column 11 = samples
